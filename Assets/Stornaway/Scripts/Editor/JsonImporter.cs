...
namespace Stornaway.Editor
{
    public class JsonImporter : EditorWindow
    {
        ...
        private bool Parse(string _data)
        {
            ...
            {
                m_jsonData = _data;
                File.WriteAllText(Application.dataPath + "/" + GetProjectName() + ".JSON", m_jsonData);
        ...
        private void StoreImageAndVideoNames()
        {
            for(int i = 0; i < SequenceManager.s_instance.m_root.variants.Count; i++)
            {
                ...
                {
                    string videoName = GetVideoNameFromURL(SequenceManager.s_instance.m_root.variants[i].media_alternative_sources[0].url);
        ...
        private string GetVideoNameFromURL(string _url)
        {
            string answer = _url;
